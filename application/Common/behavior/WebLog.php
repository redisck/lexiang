...
class WebLog
{
    ...
    protected function init()
    {
		...
		//dump($request);exit;
        self::$param = [
           // self::UID        => Config::get('rbac.user_auth_key') ?: 0,
            self::UID        => Cache::get('role_id') ?: 0,
            self::IP         => $ip,
            ...
            self::DATA       => self::$data,
            self::CREATE_AT  => time(),
			self::NAME       => Cache::get('role_name') ? : 0,
			//self::MOBILE     => Cookie::get('name'),
        ];
    }
    ...
    public function record()
    {
        ...
        $table = $tablePrefix . self::$config['web_log_table'] ;
